...
public class BaseClass 
{
   static public WebDriver driver;
...

	{
		...
		logger=LogManager.getLogger(this.getClass()); //Loading Log4j2 file and import from apache.logging.log4j
	...
	else if(p.getProperty("execution_env").equalsIgnoreCase("local"))
	{
		...
		//ChromeOptions opt= new ChromeOptions(); // rejecting cookies
	//	opt.addExtensions(new File("./Extensions/Reject_all_cookies.crx")); //file path and location
		//opt.addExtensions(new File("./Extensions/AdBlocker.crx"));
		//driver=new ChromeDriver(opt);
		
		//ChromeOptions option= new ChromeOptions(); // adding SelectorsHub while executing test
		//option.addExtensions(new File("./Extensions/SelectorsHub.crx")); //file path and location
		...
		driver.findElement(By.xpath("//p[normalize-space()='Do not consent']")).click(); //handle cookies
